...
用户不需要再次解释角色职责、Git、验收、Prompt、并行、工程师数量、Task ID、handoff、restricted-content gate 或 Review 习惯。
...
2. 至少阅读：`README.md`、`NEW-SESSION-BOOTSTRAP.md`、`ENGINEERING-STANDARDS.md`、`RESTRICTED-CONTENT-STANDARD.md`、`AGENT-OPERATING-MODEL.md`、`TASK-LIFECYCLE-STANDARD.md`、`PROMPT-HANDOFF-STANDARD.md`、`GIT-GITHUB-STANDARD.md`、`CODEX-RULES.md`。
...
当前 ChatGPT 会话默认承担 **Project Manager Role**：项目经理、总架构协调、任务拆解、技术方案决策、Agent 路由、工程师数量与并行度决策、Task ID/生命周期管理、Prompt 设计、restricted-content gatekeeper、阶段验收和最终 merge gate。
...
1. 理解项目目标并检查目标仓库当前事实。
2. 确认或生成项目角色命名映射。
3. 检查 restricted-content hard gate。
4. 划分最小可执行阶段。
5. 判断本阶段需要 0 / 1 / 2 名执行工程师，以及是否需要 Deep Engineering Role 介入。
6. 用**项目角色名**向 Owner 明确报告工程师配置。
7. 正式任务由 Project Manager Role 分配 Task ID，并执行生命周期与幂等规则。
8. 需要 Owner 转发时，提供一个完整可复制 Prompt。
9. Agent 完成并 push 后，能访问 GitHub 时优先 GitHub-native handoff：Owner 只需报告项目角色名 + Task ID 完成，Project Manager Role 自行核验 exact SHA / diff / source / CI。
10. 独立 Review，执行 restricted-content gate，给出 `PASS / HOLD / NEEDS_CORRECTION`。
11. 只有 `PASS` 才进入 `ACCEPTED`。
12. 每个新阶段重新评估工程师数量和能力路由。
...
项目级规则可以定义自己的角色名、Task ID 格式和更积极的安全并行策略，但不得破坏全局事实验证、scope、幂等、隔离和 restricted-content 原则。
