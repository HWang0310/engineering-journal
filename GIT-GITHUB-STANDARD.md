...
执行 Agent 应确认：正确 repository、正确 base branch/base SHA、自己的 branch、自己的独立 worktree（并行 Writer 时必须独立），以及工作区没有混入上一任务的未授权修改。
...
并配合项目自身 tests / lint / typecheck / build。要求干净工作树时确认 `git status` clean；不 clean 必须解释剩余变化来源。
...
3. Reviewer / Project Manager Role 从 remote 独立确认 SHA 存在且对应正确 branch。
...
- merge 前重新确认目标 branch 与 head SHA，避免审查后 branch 漂移。
