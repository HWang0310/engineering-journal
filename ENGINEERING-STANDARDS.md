...
每个任务必须明确：目标、Task ID（适用时）、允许/禁止修改范围、依赖事实与 SHA、验收标准、验证命令、Git/remote 交付要求。
...
- `PASS`：目标、验证、restricted-content gate 和 Git 证据足够，可进入 `ACCEPTED`。
- `HOLD`：实现可能正确，但缺关键验证、remote 状态、依赖或信息。
- `NEEDS_CORRECTION`：存在明确错误、越界修改、测试失败、架构偏差、restricted-content 命中或其他验收不满足。
