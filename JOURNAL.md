...
- **2026-09-20 — History Traversal / Recovery Budget：** 将长期项目 recovery 从“GitHub history 越多，fresh session 读取越多”的隐性线性成本，收敛为 **current-state-first + OPEN-first + evidence-triggered history traversal**。新增 canonical 原则：closed / merged / archived / superseded history 是 durable provenance，不是默认 working context；fresh recovery historical object 默认读取 0 个，只有 explicit current reference、unresolved contradiction、provenance/audit、regression origin、architecture rationale、migration/release ancestry、active correction delta 等 trigger 才定向扩展，并在满足 trigger 后停止。Issue/PR 编号仅为 repository-local durable sequence，不代表 active queue 大小。Active PR correction 默认恢复 current head + latest unresolved review/current handoff + delta 所需 previous reviewed SHA；merged correction history 与 historical handoff 只保留 audit/provenance。OPEN queue 应代表 actionable state；completed / superseded / registration-only Issue 及时关闭。目标是使长期项目 fresh recovery 成本接近 O(current active state)，而不是 O(total project history)。Canonical owner：`NEW-SESSION-BOOTSTRAP.md`；其它标准只保留职责内 cross-reference。

- **2026-09-15 — Project-scoped Named Dispatch：** 修正 Lean governance 与 project-scoped identity 之间的歧义。以后 PM 自己直接完成工作时可以没有 Writer；但**只要发生 Agent dispatch，就必须先从当前项目自己的 GitHub canonical roster 中解析一个具体 active named engineer identity，再单独选择 backend/profile**。`Writer` / `Reviewer` / `Agent A` 等 role labels 不能作为实际派工 identity，TeleAgent / Codex / GPT 系列 / WorkBuddy 等 backend 也不能当作工程师名字。Lean Project 可以只维护一个最小 named roster，不要求完整 multi-Agent organization；如果项目已有稳定 named engineers 但 roster 尚未落盘，应在下一次 dispatch 前先恢复并 backfill。跨项目 identity 边界继续严格保持：不得因为读取 dependency、plugin、sibling、parent/subproject、历史聊天或其它项目 handoff，就把外部 named engineer 带入当前项目。`AGENT-OPERATING-MODEL.md` 为 canonical owner，`ENGINEERING-STANDARDS.md`、`PROMPT-HANDOFF-STANDARD.md` 与 README 同步去除 generic Writer 可直接派工的旧表述。
...
- **2026-09-09 — Project-scoped Engineer Identity Boundary：** 修正 multi-repo / dependency / plugin / parent-subproject 场景中的 named engineer 跨项目串线风险。`AGENT-OPERATING-MODEL.md` §3.3 成为 canonical 定义：engineer identity 是 project-scoped durable identity；读取另一个 project/repository 的 roster 不改变 current project staffing；dependency/plugin/subproject/sibling/parent relationship 不自动共享 engineer identities；只有 current project canonical roster 中存在的 named identity 才能作为 current-project engineer dispatch，或由 Owner 明确批准 roster add。多个 repositories 只有在 project-control facts 明确声明共享 project scope / canonical roster 时才共享 identities。Cross-project bug 默认通过 project-to-project handoff，由 external project 自己 staffing，再 handback current project integration review。`NEW-SESSION-BOOTSTRAP.md` 增加 resolve current project → locate current roster → restore identities → named dispatch 顺序和 membership invariant。backend switch 仍属于 PM capability routing。
...
- **2026-09-07 — Project Roster Memory：** 为长期使用 named engineers 的项目建立 GitHub durable identity memory，防止跨会话重复命名。2026-09-15 后，任何发生 Agent dispatch 的项目至少需要一个最小 project-local named roster；Lean 只是不要求完整 multi-Agent organization。
